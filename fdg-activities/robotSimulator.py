...
class Robot():
    ...
    def calculate_intensity(self, distance, max_distance):
        ...
        return int(self.upper_intensity_val - (distance / max_distance) * (self.upper_intensity_val - self.lower_intensity_val))
    ...
    def send_json_data(self):
        ...
        if client.connect("eubbc-digital.upb.edu", 1884, 40) != 0:
        ...
        for i in range(self.height):
            for j in range(self.width):
                ...
                client.publish("street-light-rl/data", json.dumps(data)) # Publish to topic
        ...
        client.publish("street-light-rl/data", json.dumps(last_data))
        client.disconnect()
