...
class RobotGridsView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if len(queryset) == 0:
            return Response("Error: The provided Robot ID is invalid or the robot does not have any associated grids.", status=404)
        ...
        # the data is passed to a specific function for processing, which returns the corresponding grids.
            ...
            serializer_data = _handle_date_params(self, start_date_param, end_date_param, queryset)
        ...
        return Response(serializer_data) # Returns the grid(s) that match the specified criteria.
...
def _handle_grid_param(self, grid_param, queryset):
    ...
    if grid_param == "last":
        ...
        try:
            ...
            response = json.loads('{"Error": "Invalid grid number. Please enter a valid grid number within the range of available grids."}')
            return response
...
def _handle_date_params(self, start_date_param, end_date_param, queryset):
    try:
        ...
        if '-' in start_date_param and (end_date_param is not None):
            ...
            end_date_obj = (datetime.strptime(end_date_param, date_format)+ timedelta(days=1)).date()
        ...
        if start_date_obj and (end_date_param is None): 
            ...
            if start_date_obj > (current_date_obj - timedelta(days=1)): 
                response = json.loads('{"Error": "The start date cannot be later than todays date."}')
                return response
            filtered_queryset = queryset.filter(timestamp__range=[start_date_obj, current_date_obj])
            ...
            if start_date_obj > (end_date_obj - timedelta(days=1)):
                response = json.loads('{"Error": "The start date cannot be after the end date."}')
                return response
            # Filters the queryset to include grids with dates within the specified start and end date range.
            filtered_queryset = queryset.filter(timestamp__range=[start_date_obj, end_date_obj]) 
