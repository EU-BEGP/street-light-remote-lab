...
export class CameraMovementService {
  ...
  moveCamera(direction: string, camera_number: number): Observable<any> {
    ...
    return this.http.post(`${this.URL}${config.api.streetLights.cameraMovement.move}`, body, this.httpOptions);
  }

  stopCamera(camera_number: number): Observable<any> {
    const body = { camera_number };
    return this.http.post(`${this.URL}${config.api.streetLights.cameraMovement.stop}`, body, this.httpOptions);
  }
}
