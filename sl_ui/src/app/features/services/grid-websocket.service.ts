...
export const WS_URL = config.api.websocketUrls.gridInformation;
...
export class GridWebsocketService {
  ...
  public connect(): void {
    ...
    if (!this.socket$ || this.socket$.closed) {
      ...
      this.messageSubscription = this.socket$.pipe(
        catchError(error => {
          console.error('[GridWebsocketService] Error:', error);
          return EMPTY;
        })
      ).subscribe(
        message => this.messagesSubject$.next(message),
        error => console.error('[GridWebsocketService] Received error:', error),
        (): void => { }
      );
    }
  }
  ...
  private reconnect(): void {
    console.log('[GridWebsocketService] Attempting to reconnect...');
    setTimeout((): void => {
  ...
  private getNewWebSocket(wsUrl: string): WebSocketSubject<any> {
    return webSocket({
      url: wsUrl,
      openObserver: {
        next: (): void => console.log('[GridWebsocketService] WebSocket connection established'),
      },
      closeObserver: {
        next: (): void => {
          console.log('[GridWebsocketService] WebSocket connection closed');
